...
export default class OLGame extends Scene {
    init(__myArr, __enemyArr) {
        ...
        this.com_blood.init();
        //另一个玩家卡牌组
        this.com_cardArr = getComCardArr(__enemyArr);
    ...
    begin() {
        ...
        this.btn.on('pointerdown', () => {
            this.btn.visible = false;
            this.user_ok = true;
            global.socket.myOk(this.user_card.id);
    ...
    socketEvent() {
        //另一个玩家已出牌
        global.socket.emitter.on(global.socket.PLAY_OK, (__index) => {
            console.log('444444');
            this.comSelectCard(__index);
        });
    }
    ...
    //另一个玩家出牌
    comSelectCard(__index) {
        console.log(__index);
        console.log(this.com_cardArr);
        let obj = this.com_cardArr[__index];
        console.log(obj);
